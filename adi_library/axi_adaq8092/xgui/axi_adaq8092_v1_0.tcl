...
source [file join [file dirname [file dirname [info script]]] gui/axi_adaq8092_v1_0.gtcl]
...
proc init_gui { IPINST } {
  ...
  ipgui::add_param $IPINST -name "OUTPUT_MODE" -parent ${Page_0}
  ipgui::add_param $IPINST -name "POLARITY_MASK" -parent ${Page_0}
  #Adding Group
...
proc update_PARAM_VALUE.POLARITY_MASK { PARAM_VALUE.POLARITY_MASK PARAM_VALUE.OUTPUT_MODE } {
	# Procedure called to update POLARITY_MASK when any of the dependent parameters in the arguments change
	
	set POLARITY_MASK ${PARAM_VALUE.POLARITY_MASK}
	set OUTPUT_MODE ${PARAM_VALUE.OUTPUT_MODE}
	set values(OUTPUT_MODE) [get_property value $OUTPUT_MODE]
	if { [gen_USERPARAMETER_POLARITY_MASK_ENABLEMENT $values(OUTPUT_MODE)] } {
		set_property enabled true $POLARITY_MASK
	} else {
		set_property enabled false $POLARITY_MASK
	}
}

proc validate_PARAM_VALUE.POLARITY_MASK { PARAM_VALUE.POLARITY_MASK } {
	# Procedure called to validate POLARITY_MASK
	return true
}
...
proc update_PARAM_VALUE.OUTPUT_MODE { PARAM_VALUE.OUTPUT_MODE } {
	# Procedure called to update OUTPUT_MODE when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.OUTPUT_MODE { PARAM_VALUE.OUTPUT_MODE } {
	# Procedure called to validate OUTPUT_MODE
	return true
}
...
proc update_MODELPARAM_VALUE.OUTPUT_MODE { MODELPARAM_VALUE.OUTPUT_MODE PARAM_VALUE.OUTPUT_MODE } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.OUTPUT_MODE}] ${MODELPARAM_VALUE.OUTPUT_MODE}
}

proc update_MODELPARAM_VALUE.POLARITY_MASK { MODELPARAM_VALUE.POLARITY_MASK PARAM_VALUE.POLARITY_MASK } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.POLARITY_MASK}] ${MODELPARAM_VALUE.POLARITY_MASK}
}
