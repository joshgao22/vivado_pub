 ...
 */
int32_t AD9528_spiWriteByte(ad9528Device_t *device, uint16_t addr,
				 uint8_t data)
{
	uint8_t buf[3];
	int32_t status;
	...
	buf[1] = addr & 0xFF;
	buf[2] = data;
	status = spi_write_and_read_3wire(device->spi_desc,
			buf, NULL, 3, 0);
 ...
 */
int32_t AD9528_spiReadByte(ad9528Device_t *device, uint16_t addr,
				uint8_t *readdata)
{
	uint8_t tx_buf[2];
	uint8_t rx_buf[1];
 ...
 */
int32_t AD9528_spiWriteField(ad9528Device_t *device,
 ...
 */
int32_t AD9528_setTimeout_ms(uint32_t timeOut_ms)
{
 ...
 */
int32_t AD9528_hasTimeoutExpired()
{
	udelay(1000);
 ...
 */
int32_t AD9528_resetDevice(ad9528Device_t *device)
{
 ...
 * \return Returns enum ADI_ERR, ADIERR_OK=pass, !ADIERR_OK=fail
 */
int32_t AD9528_setSpiSettings(ad9528Device_t *device)
{
	uint8_t spiReg = 0;
...
*/
int32_t AD9528_initDeviceDataStruct(ad9528Device_t *device,
...
//			~0x318A);	 //enable outputs 13,12,8,7,3 and 1
//
//	for (i=0; i<14; i++) {
...
//	device->outputSettings->outSource[7] = SYSREF;
...
//		ONE_PULSE;		//ONE_PULSE=0, TWO_PULSES=2, FOUR_PULSES=3, SIX_PULSES=4, EIGHT_PULSES=5
...
int32_t AD9528_init(ad9528Device_t **device, const ad9528Init_t *init_device)
{
	int ret = SUCCESS;
	ad9528Device_t *dev;

	dev = (ad9528Device_t *)malloc(sizeof(*dev));
	if (!dev)
		return FAILURE;

	/* SPI */
	ret = spi_init(&dev->spi_desc, init_device->spi_init);
	ret |= gpio_get(&dev->gpio_resetb, init_device->gpio_init_resetb);
	ret |= gpio_get(&dev->gpio_sysref_req, init_device->gpio_init_sysref_req);

	gpio_direction_output(dev->gpio_resetb, 0);
	gpio_direction_output(dev->gpio_sysref_req, 0);

	AD9528_resetDevice(dev);

	dev->spiSettings = init_device->spiSettings;
	dev->pll1Settings = init_device->pll1Settings;
	dev->pll2Settings = init_device->pll2Settings;
	dev->outputSettings = init_device->outputSettings;
	dev->sysrefSettings = init_device->sysrefSettings;
 ...
 * \return Returns enum ADI_ERR, 0=pass, >0 = fail
 */
int32_t AD9528_config(ad9528Device_t
 ...
 * \return Returns enum ADI_ERR, 0=pass, >0 = fail
 ***********************************************************************************/
int32_t AD9528_requestSysref(ad9528Device_t *device, uint8_t enableSYSREF)
{
 ...
 * \return Returns enum ADI_ERR, 0=pass, >0 = fail
 ***********************************************************************************/
int32_t AD9528_setupSYSREF(ad9528Device_t *device,
...
int32_t AD9528_sysrefReq(ad9528Device_t *device,
 ...
 */
int32_t AD9528_remove(ad9528Device_t *device)
{
	int32_t status;
